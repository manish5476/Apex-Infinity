...
@Component({
  ...
  imports: [
    ...
    AgShareGrid // Using your custom grid
  ],
  template: `
      ...
      <ng-container *ngIf="!loading(); else loader">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <ng-container *ngIf="financialData()?.recommendations?.recommendations?.length > 0">
               <div class="mt-6 p-3 flex items-center gap-3 border border-dashed" 
                 ...
                 <p [style.color]="'var(--theme-text-secondary)'" [style.font-size]="'var(--font-size-xs)'">
                   <strong>Action:</strong> {{ financialData()?.recommendations?.recommendations[0]?.action }} 
                   <span class="opacity-70">- {{ financialData()?.recommendations?.recommendations[0]?.reason }}</span>
                 </p>
               </div>
            </ng-container>
          </div>
          ...
          <div class="p-6 border flex flex-col justify-between" 
               [style.background]="'var(--theme-bg-secondary)'" [style.border-color]="'var(--theme-border-primary)'" [style.border-radius]="'var(--ui-border-radius-xl)'">
            <div>
              <h3 class="font-bold uppercase tracking-widest mb-4" [style.color]="'var(--theme-text-label)'" [style.font-size]="'var(--font-size-xs)'">Cash Flow Sources</h3>
              
              <div class="space-y-4">
                @for (mode of financialData()?.cashFlow?.paymentModes; track mode.name) {
                  <div>
                    <div class="flex justify-between mb-1">
                      <span class="capitalize font-bold" [style.color]="'var(--theme-text-primary)'" [style.font-size]="'var(--font-size-sm)'">{{ mode.name }}</span>
                      <span class="font-bold tabular-nums text-emerald-400">₹{{ mode.value | number }}</span>
                    </div>
                ...
                @if (!financialData()?.cashFlow?.paymentModes?.length) {
                  <p class="text-center py-4 text-xs italic" [style.color]="'var(--theme-text-tertiary)'">No cash flow data recorded.</p>
                }
              </div>
            </div>
            
            <div class="mt-4 pt-4 border-t" [style.border-color]="'var(--theme-border-primary)'">
               <p [style.color]="'var(--theme-text-tertiary)'" [style.font-size]="'var(--font-size-xs)'" class="mb-1">Net Payable Tax</p>
               <p class="text-xl font-bold tabular-nums text-rose-400">₹{{ financialData()?.tax?.netPayable | number }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="lg:col-span-8 p-6 border transition-colors" 
                ...
                <div class="md:col-span-3 space-y-4">
                  ...
                  <div class="flex gap-6">
                    <div class="flex items-center gap-2">
                      ...
                      <span [style.color]="'var(--theme-text-secondary)'" [style.font-size]="'var(--font-size-xs)'">Default Rate: {{ emi.defaultRate | number:'1.1-1' }}%</span>
                    </div>
                    <div class="flex items-center gap-2">
            ...
            @if (!financialData()?.credit?.emiAnalytics?.length) {
                <div class="flex flex-col items-center justify-center py-8 text-center opacity-60">
                    <i class="pi pi-check-circle text-4xl mb-2 text-emerald-500"></i>
                    <p class="text-sm font-bold text-white">No Active Credit Lines</p>
                    <p class="text-xs text-white/50">There are no active EMIs or pending credit installments.</p>
                </div>
            }
          </div>
          ...
          <div class="lg:col-span-4 space-y-6">
            ...
            <div class="border overflow-hidden flex flex-col h-[300px]" 
                 [style.background]="'var(--theme-bg-secondary)'" 
                 [style.border-color]="'var(--theme-border-primary)'" 
                 [style.border-radius]="'var(--ui-border-radius-xl)'">
               <div class="p-4 border-b shrink-0" [style.border-color]="'var(--theme-border-primary)'" [style.background]="'var(--theme-bg-ternary)'">
                  <h4 class="font-bold uppercase tracking-tighter" [style.color]="'var(--theme-text-label)'" [style.font-size]="'var(--font-size-xs)'">Receivables Aging</h4>
               </div>
               
               <div class="grid-wrapper relative flex-1">
                  <app-ag-share-grid 
            ...
            <div class="p-5 border transition-colors shadow-inner" [style.background]="'var(--theme-bg-secondary)'" [style.border-color]="'var(--theme-border-primary)'" [style.border-radius]="'var(--ui-border-radius-xl)'">
              ...
              <div class="space-y-3">
                ...
                @if (!financialData()?.recommendations?.recommendations?.length) {
                    <p class="text-xs italic text-center py-4 opacity-50">No urgent actions required.</p>
                }
              </div>
            </div>
          </div>
  ...
  setupAgingColumns(): void {
    this.agingColumns = [
        ...
        },
        {
            ...
            cellStyle: { 'font-weight': '700', 'color': 'var(--theme-error)', 'text-align': 'right' }
        }
    ];
