...
<div class="shippit-layout">
      ...
      <div class="header-titles">
        ...
        <p class="text-muted">
          @if (dashboard()?.period) {
            ...
            <span class="badge badge-gray ml-2">{{ dashboard()!.period.days }} Days</span>
          } @else {
            &mdash;
          }
        </p>
    ...
    <div class="header-right">
      ...
      <div class="filter-box">
        <p-datepicker
          ...
          styleClass="clean-datepicker">
        </p-datepicker>
      </div>

      @if (dashboard()?.financial?.performance?.executionTime) {
        <div class="exec-time" pTooltip="Query Time" tooltipPosition="bottom">
          <i class="pi pi-bolt text-warning"></i> {{ dashboard()!.financial.performance.executionTime }}
        </div>
      }

      <button class="btn-icon" (click)="loadDashboard()" [disabled]="loading()" pTooltip="Sync Data">
    ...
    <div class="dashboard-content">
      ...
      @if ((dashboard()!.alerts.lowStockCount ?? 0) > 0) {
        <div class="alert-banner">
          ...
          <div class="alert-text flex-1">
            <h4>Inventory Warning</h4>
            <p><strong>{{ dashboard()!.alerts.lowStockCount }} items</strong> below reorder level. Action required.</p>
          </div>
          <div class="alert-tags">
            @for (item of dashboard()!.alerts.itemsToReorder.slice(0, 2); track item) {
              <span class="badge badge-warning-outline">{{ item }}</span>
            }
            @if (dashboard()!.alerts.itemsToReorder.length > 2) {
              <span class="badge badge-warning-outline">+{{ dashboard()!.alerts.itemsToReorder.length - 2 }} MORE</span>
            }
          </div>
        </div>
      }
      ...
      <div class="kpi-grid">
        <div class="admin-card kpi-card">
          <div class="kpi-head">
            ...
            <div class="icon-box bg-blue"><i class="pi pi-wallet"></i></div>
          </div>
          <div class="kpi-value">{{ dashboard()!.financial.totalRevenue.value | currency:'INR':'symbol':'1.0-0' }}</div>
          <div class="kpi-foot">
            <span class="badge badge-success"><i class="pi pi-arrow-up-right"></i> {{ dashboard()!.financial.totalRevenue.growth }}%</span>
            <span class="text-muted text-sm ml-2">{{ dashboard()!.financial.totalRevenue.count }} transactions</span>
          </div>
        </div>
        ...
        <div class="admin-card kpi-card">
          <div class="kpi-head">
            ...
            <div class="icon-box" [ngClass]="dashboard()!.financial.netProfit.status === 'profitable' ? 'bg-green' : 'bg-red'">
              <i class="pi pi-chart-line"></i>
            </div>
          </div>
          <div class="kpi-value" [ngClass]="dashboard()!.financial.netProfit.status === 'profitable' ? 'text-success' : 'text-danger'">
            {{ dashboard()!.financial.netProfit.value | currency:'INR':'symbol':'1.0-0' }}
          </div>
          <div class="kpi-foot align-center">
            <div class="progress-bar flex-1">
              <div class="progress-fill" [ngClass]="dashboard()!.financial.netProfit.status === 'profitable' ? 'fill-green' : 'fill-red'" [style.width.%]="Math.min(Math.max(dashboard()!.financial.netProfit.margin, 0), 100)"></div>
            </div>
            <span class="text-muted text-sm ml-2 font-mono">{{ dashboard()!.financial.netProfit.margin | number:'1.1-1' }}%</span>
          </div>
        </div>
        ...
        <div class="admin-card kpi-card">
          <div class="kpi-head">
            ...
            <div class="icon-box bg-orange"><i class="pi pi-box"></i></div>
          </div>
          <div class="kpi-value">{{ dashboard()!.inventory.summary.valuation | currency:'INR':'symbol':'1.0-0' }}</div>
          <div class="kpi-foot">
            <span class="text-muted text-sm">{{ dashboard()!.inventory.inventoryValuation.totalItems }} items across {{ dashboard()!.inventory.inventoryValuation.productCount }} SKUs</span>
          </div>
        </div>
        ...
        <div class="admin-card kpi-card">
          <div class="kpi-head">
            ...
            <div class="icon-box bg-purple"><i class="pi pi-heart-fill"></i></div>
          </div>
          <div class="kpi-health-wrapper">
            <svg viewBox="0 0 36 36" class="circular-chart">
              <path class="circle-bg" d="M18 2.0845 a15.9155 15.9155 0 0 1 0 31.831 a15.9155 15.9155 0 0 1 0-31.831" />
              <path class="circle" [attr.stroke-dasharray]="(dashboard()!.inventory.healthScore || 0) + ', 100'" [ngClass]="getHealthClass(dashboard()!.inventory.healthScore)" d="M18 2.0845 a15.9155 15.9155 0 0 1 0 31.831 a15.9155 15.9155 0 0 1 0-31.831" />
            </svg>
            <div class="health-percentage">{{ dashboard()!.inventory.healthScore || 0 }}%</div>
          </div>
        </div>
      </div>
        ...
        <div class="bento-left">
          ...
          <div class="admin-card">
            <div class="card-header">
              ...
              <span class="badge badge-gray">{{ dashboard()!.insights.count }} NEW</span>
            </div>
            <div class="insights-list">
              @for (insight of dashboard()!.insights.insights; track insight.title) {
                <div class="insight-row" [ngClass]="getInsightClass(insight)">
                  <div class="insight-icon"><i class="pi" [ngClass]="getInsightIcon(insight)"></i></div>
                  <div class="insight-body flex-1">
                    <div class="flex-between">
                      <span class="insight-title">{{ insight.title }}</span>
                      <span class="badge" [ngClass]="insight.priority === 'high' ? 'badge-danger' : 'badge-gray'">{{ insight.priority | uppercase }}</span>
                    </div>
                    <p class="insight-desc">{{ insight.message }}</p>
                  </div>
                </div>
              }
            </div>
                ...
                @for (cat of dashboard()!.topCategories; track cat.name) {
                  <div class="category-item">
                    <div class="cat-left">
                      ...
                      <span class="cat-margin font-mono">{{ cat.margin | number:'1.1-1' }}% margin</span>
                    </div>
                    <div class="cat-right flex-1">
                      <div class="metric-row">
                        <span class="m-label">REVENUE</span>
                        <div class="progress-bar flex-1"><div class="progress-fill fill-blue" style="width:100%"></div></div>
                        <span class="m-value font-mono">{{ cat.revenue | currency:'INR':'symbol':'1.0-0' }}</span>
                      </div>
                      <div class="metric-row">
                        <span class="m-label">PROFIT</span>
                        <div class="progress-bar flex-1"><div class="progress-fill fill-green" [style.width.%]="Math.max((cat.profit / cat.revenue) * 100, 0)"></div></div>
                        <span class="m-value font-mono">{{ cat.profit | currency:'INR':'symbol':'1.0-0' }}</span>
                      </div>
                    </div>
                  </div>
                }
        ...
        <div class="bento-right">
          ...
          <div class="admin-card">
            ...
            <div class="list-group">
              <div class="list-item">
                <span class="item-label">Avg Order Value</span>
                <span class="item-value font-mono">{{ dashboard()!.operations.orderEfficiency.averageOrderValue | currency:'INR':'symbol':'1.0-0' }}</span>
              </div>
              <div class="list-item">
                <span class="item-label">Discount Rate</span>
                <span class="item-value font-mono">{{ dashboard()!.operations.discountMetrics.discountRate }}%</span>
              </div>
              <div class="list-item">
                <span class="item-label">Cancellation Rate</span>
                <span class="item-value font-mono text-danger">{{ dashboard()!.operations.orderEfficiency.cancellationRate }}%</span>
              </div>
              <div class="list-item">
                <span class="item-label">Active Customers</span>
                <span class="item-value font-mono">{{ dashboard()!.financial.customers.active }}</span>
              </div>
              <div class="list-item">
                <span class="item-label">New Customers</span>
                <span class="badge badge-success">+{{ dashboard()!.financial.customers.new }}</span>
              </div>
            </div>
          </div>
                ...
                @for (prod of dashboard()!.leaders.topProducts; track prod._id; let i = $index) {
                  <div class="list-item-detailed">
                    ...
                    <div class="details flex-1">
                      ...
                      <span class="subtitle">{{ prod.soldQty }} sold</span>
                    </div>
                    <div class="amount font-mono text-main">{{ prod.revenue | currency:'INR':'symbol':'1.0-0' }}</div>
                  </div>
                }
              </div>
            </div>
            ...
            <div class="admin-card">
              ...
              <div class="list-group list-group-flush">
                @for (cust of dashboard()!.leaders.topCustomers; track cust._id) {
                  <div class="list-item-detailed">
                    <div class="avatar" [style.background-color]="getAvatarColor(cust._id)">{{ getInitials(cust.name) }}</div>
                    <div class="details flex-1">
                      <span class="title">{{ cust.name }}</span>
                      <span class="subtitle">{{ cust.transactions }} orders</span>
                    </div>
                    <div class="amount font-mono text-main">{{ cust.totalSpent | currency:'INR':'symbol':'1.0-0' }}</div>
                  </div>
                }
              </div>
            </div>
  ...
  styles: [`
    /* ==========================================================================
       HARDCODED PREMIUM "SHIPPIT/SKYRISE" THEME
       Overrides global CSS to guarantee absolute visual fidelity.
       ========================================================================== */

    .shippit-layout {
      background-color: #F3F4F6 !important; /* Enforces the light gray background */
      min-height: 100vh;
    ...
    .font-mono { font-family: 'Fira Code', ui-monospace, monospace !important; }
    ...
    .header-titles h1 { margin: 0 0 4px 0 !important; font-size: 1.25rem !important; font-weight: 700 !important; color: #111827 !important; }
    .header-titles p { margin: 0 !important; font-size: 0.85rem !important; font-weight: 500 !important; display: flex; align-items: center; }
    
    .header-right { display: flex; align-items: center; gap: 12px; }
    .filter-box { background: #FFFFFF !important; border-radius: 8px !important; }
    
    .exec-time {
      background-color: #F9FAFB !important; border: 1px solid #E5E7EB !important;
      padding: 8px 12px; border-radius: 8px !important; font-size: 0.85rem !important;
      font-weight: 600 !important; color: #4B5563 !important; font-family: monospace;
    }

    .btn-icon {
      background-color: #FFFFFF !important; border: 1px solid #E5E7EB !important;
      color: #4B5563 !important; width: 40px; height: 40px; border-radius: 8px !important;
      cursor: pointer; display: flex; align-items: center; justify-content: center;
      transition: all 0.2s !important;
    }
    .btn-icon:hover:not([disabled]) { background-color: #F3F4F6 !important; color: #111827 !important; }
    ...
    /* Base Card Style (Forces White Background and Borders) */
    .admin-card {
      background-color: #FFFFFF !important;
    ...
    .card-header {
      padding: 20px 24px; border-bottom: 1px solid #E5E7EB !important;
      display: flex; justify-content: space-between; align-items: center;
    }
    .card-header h2 { margin: 0 !important; font-size: 1rem !important; font-weight: 700 !important; color: #111827 !important; }

    /* Badges */
    .badge {
      display: inline-flex; align-items: center; gap: 4px;
      padding: 4px 10px; border-radius: 9999px !important;
      font-size: 0.75rem !important; font-weight: 700 !important;
      text-transform: uppercase; letter-spacing: 0.05em;
    }
    .badge-gray { background-color: #F3F4F6 !important; color: #4B5563 !important; border: 1px solid #E5E7EB !important; }
    .badge-success { background-color: #DCFCE7 !important; color: #059669 !important; border: 1px solid #BBF7D0 !important; }
    .badge-danger { background-color: #FEE2E2 !important; color: #DC2626 !important; border: 1px solid #FECACA !important; }
    .badge-warning-outline { background-color: transparent !important; color: #92400E !important; border: 1px solid #FDE68A !important; }

    /* Alert Banner */
    .alert-banner {
      background-color: #FEF3C7 !important; border: 1px solid #FDE68A !important;
      border-radius: 16px !important; padding: 20px 24px;
      display: flex; align-items: center; gap: 16px; flex-wrap: wrap;
    }
    .alert-icon { font-size: 1.5rem; color: #D97706 !important; }
    .alert-text h4 { margin: 0 0 4px 0 !important; font-size: 1rem !important; color: #92400E !important; font-weight: 700 !important; }
    .alert-text p { margin: 0 !important; font-size: 0.9rem !important; color: #B45309 !important; }
    ...
    .kpi-title { font-size: 0.8rem !important; font-weight: 700 !important; color: #6B7280 !important; letter-spacing: 0.05em; }
    
    /* Icon Boxes */
    .icon-box { width: 44px; height: 44px; border-radius: 12px !important; display: flex; align-items: center; justify-content: center; font-size: 1.25rem; }
    .bg-blue { background-color: #EFF6FF !important; color: #2563EB !important; }
    .bg-green { background-color: #DCFCE7 !important; color: #10B981 !important; }
    .bg-red { background-color: #FEE2E2 !important; color: #EF4444 !important; }
    .bg-orange { background-color: #FFF7ED !important; color: #EA580C !important; }
    .bg-purple { background-color: #F3E8FF !important; color: #A855F7 !important; }

    .kpi-value { font-size: 2rem !important; font-weight: 800 !important; color: #111827 !important; line-height: 1 !important; font-family: ui-monospace, monospace !important; letter-spacing: -0.02em; }
    .kpi-foot { display: flex; align-items: center; }
    .align-center { align-items: center; }

    /* Progress Bars */
    .progress-bar { height: 6px; background-color: #F3F4F6 !important; border-radius: 999px !important; overflow: hidden; }
    .progress-fill { height: 100%; border-radius: 999px !important; transition: width 0.5s ease; }
    .fill-green { background-color: #10B981 !important; }
    .fill-red { background-color: #EF4444 !important; }
    .fill-blue { background-color: #3B82F6 !important; }

    /* Health Ring */
    .kpi-health-wrapper { position: relative; width: 64px; height: 64px; align-self: flex-end; margin-top: -30px; }
    .circular-chart { display: block; margin: 0 auto; max-width: 80%; max-height: 250px; }
    .circle-bg { fill: none; stroke: #F3F4F6 !important; stroke-width: 3.8; }
    .circle { fill: none; stroke-width: 3.8; stroke-linecap: round; transform: rotate(-90deg); transform-origin: 18px 18px; transition: stroke-dasharray 1s ease; }
    .health-percentage { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-family: monospace; font-size: 0.85rem !important; font-weight: 800 !important; color: #111827 !important; }
    
    .health-good { stroke: #10B981 !important; }
    .health-warn { stroke: #F59E0B !important; }
    .health-bad { stroke: #EF4444 !important; }
    ...
    .bento-left, .bento-right { display: flex; flex-direction: column; gap: 24px; }

    /* AI Insights */
    .insights-list { display: flex; flex-direction: column; }
    .insight-row { display: flex; gap: 16px; padding: 20px 24px; border-bottom: 1px solid #E5E7EB !important; background-color: #FFFFFF !important; border-left: 4px solid transparent !important; }
    .insight-row:last-child { border-bottom: none !important; }
    
    .insight-row.positive { border-left-color: #10B981 !important; background-color: #F8FAFC !important; }
    .insight-row.warning { border-left-color: #F59E0B !important; }
    .insight-row.info { border-left-color: #3B82F6 !important; }
    .insight-row.danger { border-left-color: #EF4444 !important; background-color: #FEF2F2 !important; }
    
    .insight-icon { font-size: 1.25rem; margin-top: 2px; }
    .insight-title { font-weight: 700 !important; font-size: 0.95rem !important; color: #111827 !important; }
    .insight-desc { margin: 4px 0 0 0 !important; font-size: 0.85rem !important; color: #4B5563 !important; line-height: 1.5; }

    /* Category Performance */
    .category-grid { display: flex; flex-direction: column; }
    .category-item { display: flex; align-items: center; gap: 24px; padding: 20px 24px; border-bottom: 1px solid #E5E7EB !important; }
    .category-item:last-child { border-bottom: none !important; }
    @media (max-width: 600px) { .category-item { flex-direction: column; align-items: flex-start; gap: 16px; } }
    
    .cat-left { width: 160px; display: flex; flex-direction: column; gap: 4px; flex-shrink: 0; }
    .cat-name { font-weight: 700 !important; font-size: 0.95rem !important; color: #111827 !important; }
    .cat-margin { font-size: 0.75rem !important; color: #6B7280 !important; }
    
    .cat-right { display: flex; flex-direction: column; gap: 12px; }
    .metric-row { display: flex; align-items: center; gap: 16px; }
    .m-label { width: 60px; font-size: 0.7rem !important; font-weight: 700 !important; color: #6B7280 !important; }
    .m-value { width: 80px; text-align: right; font-size: 0.85rem !important; font-weight: 700 !important; color: #111827 !important; }

    /* AG Grid Wrapper */
    .grid-container { height: 350px; width: 100%; background-color: #FFFFFF !important; }

    /* Side Panel Lists */
    .list-group { display: flex; flex-direction: column; padding: 16px 24px; gap: 16px; }
    .list-item { display: flex; justify-content: space-between; align-items: center; padding-bottom: 12px; border-bottom: 1px dashed #E5E7EB !important; }
    .list-item:last-child { border-bottom: none !important; padding-bottom: 0; }
    .item-label { font-size: 0.9rem !important; color: #4B5563 !important; font-weight: 500 !important; }
    .item-value { font-size: 0.95rem !important; font-weight: 700 !important; color: #111827 !important; }

    .list-group-flush { padding: 0; gap: 0; }
    .list-item-detailed { display: flex; align-items: center; gap: 16px; padding: 16px 24px; border-bottom: 1px solid #E5E7EB !important; }
    .list-item-detailed:last-child { border-bottom: none !important; }
    
    .rank { width: 24px; font-size: 0.8rem !important; font-weight: 700 !important; color: #9CA3AF !important; }
    .avatar { width: 36px; height: 36px; border-radius: 50% !important; display: flex; align-items: center; justify-content: center; font-size: 0.85rem !important; font-weight: 700 !important; color: #FFFFFF !important; }
    .details { display: flex; flex-direction: column; gap: 2px; }
    .title { font-weight: 600 !important; font-size: 0.9rem !important; color: #111827 !important; }
    .subtitle { font-size: 0.75rem !important; color: #6B7280 !important; }
    .amount { font-size: 0.9rem !important; font-weight: 700 !important; }

    /* PrimeNG Overrides - Forcing styles ignoring global themes */
    ::ng-deep .clean-select .p-select,
    ::ng-deep .clean-datepicker .p-datepicker-input {
      background-color: #FFFFFF !important;
    ...
    ::ng-deep .clean-select .p-select:hover,
    ::ng-deep .clean-datepicker .p-datepicker-input:hover { border-color: #9CA3AF !important; }
    ::ng-deep .clean-select .p-select.p-focus,
    ::ng-deep .clean-datepicker .p-datepicker-input:focus { border-color: #2563EB !important; box-shadow: 0 0 0 2px rgba(37,99,235,0.2) !important; }
    ::ng-deep .clean-select { min-width: 180px; }

    /* Loader */
    .loader-overlay { flex: 1; display: flex; align-items: center; justify-content: center; min-height: 500px; }
    .loader-box { background: #FFFFFF !important; padding: 32px 48px; border-radius: 16px !important; border: 1px solid #E5E7EB !important; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05) !important; display: flex; flex-direction: column; align-items: center; gap: 16px; }
    .loader-box span { font-size: 0.85rem !important; font-weight: 700 !important; color: #6B7280 !important; text-transform: uppercase; letter-spacing: 0.05em; }
...
export class AdminDashboardUiComponent implements OnInit, OnDestroy {
  ...
  loading = signal(true);
  Math = Math; // Provide Math object to template
  ...
  setupColumns(): void {
    // Hardcoded styling to ensure the AG Grid looks clean inside the white card
    this.alertColumns = [
      {
        field: 'name',
        headerName: 'Item Name',
        flex: 2,
        cellStyle: { 'font-weight': '600', 'color': '#111827', 'font-size': '13px' }
      },
      {
        field: 'sku',
        headerName: 'SKU',
        flex: 1,
        cellStyle: { 'font-family': 'monospace', 'color': '#6B7280', 'font-size': '12px' }
      },
      {
        field: 'currentStock',
        headerName: 'Stock',
      ...
      },
      {
        ...
        cellRenderer: (p: any) => {
          ...
          return `<span style="
            ...
            text-transform: uppercase; letter-spacing: 0.5px; display: inline-block; line-height: 1;
          ">${p.value}</span>`;
        }
      }
    ];
  ...
  getInsightClass(insight: any): string {
    if (insight.type === 'positive') return 'positive';
    if (insight.type === 'warning') return 'warning';
    if (insight.type === 'info') return 'info';
    if (insight.priority === 'high') return 'danger';
    return '';
  }

  getInsightIcon(insight: any): string {
    if (insight.type === 'positive') return 'pi-check-circle';
    if (insight.type === 'warning') return 'pi-exclamation-triangle';
    if (insight.type === 'info') return 'pi-info-circle';
    if (insight.priority === 'high') return 'pi-bell';
    return 'pi-circle';
  }
}// import {
//   Component, OnInit, signal, computed,
