      ...
      <div class="mb-8 flex flex-wrap justify-between items-end gap-4">
        ...
        <div class="flex flex-col items-end gap-2">
           <div class="flex gap-2">
             ...
             <p-button label="Recalculate" icon="pi pi-refresh" severity="info" size="small" (onClick)="loadForecast()" [loading]="loading()"></p-button>
           </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="p-8 border relative overflow-hidden" 
              ...
              <div class="flex justify-between items-start relative z-10">
                <div>
                  ...
                  <h1 class="text-5xl font-bold tabular-nums my-2" [style.color]="'var(--theme-text-primary)'">
                    ₹{{ forecastData()?.forecast[0]?.predictedRevenue | number:'1.0-0' }}
                  </h1>
                </div>
              ...
              <div class="mt-10 relative z-10">
                <div class="flex justify-between text-[11px] font-bold uppercase mb-2" [style.color]="'var(--theme-text-label)'">
                  <span>Lower Bound: ₹{{ forecastData()?.forecast[0]?.lowerBound | number:'1.0-0' }}</span>
                  <span>Confidence: {{ (currentFilters.confidence || 0.95) * 100 }}%</span>
                  <span>Upper Bound: ₹{{ forecastData()?.forecast[0]?.upperBound | number:'1.0-0' }}</span>
                </div>
            ...
            <div class="p-5 border border-dashed flex items-start gap-4"
               ...
               <div>
                 ...
                 <p class="mt-1 leading-relaxed" [style.color]="'var(--theme-text-secondary)'" [style.font-size]="'var(--font-size-xs)'">
                   The model predicts a <span class="font-bold text-rose-400">{{ forecastData()?.forecast[0]?.growth }}% change</span> for the next period. 
                   Review inventory reorder levels for high-cost SKUs to maintain optimal cash flow.
                 </p>
               </div>
            </div>
          </div>
          ...
          <div class="lg:col-span-4 space-y-6">
            ...
            <div class="p-6 border text-center" 
              ...
              <p class="text-4xl font-bold text-emerald-500 tabular-nums my-2">{{ (currentFilters.confidence || 0.95) * 100 }}%</p>
...
export class SalesForecastComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    // loadForecast triggered by filter init
  }
