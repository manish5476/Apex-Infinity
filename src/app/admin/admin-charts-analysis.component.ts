...
@Component({
      ...
      @if (previewChart()) {
        <div class="dlg-root">
          ...
          <div class="dlg-footer">
            <div class="dlg-thumbs custom-scrollbar">
  ...
  styles: [`
    ...
    /* Dialog Body - Ensuring the map/chart respects boundaries */
    .dlg-body {
      flex: 1;
      padding: var(--spacing-2xl);
      background: var(--bg-secondary);
...
export class AdminChartsAnalysisComponent {
  ...
  chartReady     = signal(false); // Controls exactly when the child chart renders
  ...
  openPreview(chart: ChartMeta): void {
    ...
    this.chartReady.set(false); // Hide the chart instance initially
    this.dialogVisible.set(true);
  }

  onDialogShow(): void {
    // Dialog transform animation is completed. The container bounds are final.
    // It is now perfectly safe to inject and render the chart.
  ...
  navigateChart(dir: -1 | 1): void {
    ...
    if (next >= 0 && next < charts.length) {
      this.chartReady.set(false); // Temporarily hide to force recalculation
      this.previewChart.set(charts[next]);
      
      // Briefly yield to the event loop before re-rendering the next chart
      setTimeout(() => this.chartReady.set(true), 50);
    }
  }
