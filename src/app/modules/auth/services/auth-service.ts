...
export class AuthService {
  ...
  private validateTokenOnLoad(): void {
    if (isPlatformBrowser(this.platformId)) {
      ...
      // ✅ FIX: Remove the `!this.currentUserValue` check.
      // We MUST validate the token on every page load if it exists,
      // to refresh the user data and check if the token is still valid.
      if (token) {
        ...
        this.apiService.getMe().subscribe({
          next: (user) => {
            ...
            // ✅ This is the line that unblocks the auth guard
            this.currentUserSubject.next(user);

            // ✅ Connect socket *after* successful validation
            if (user?._id) {
          ...
          error: () => {
            ...
            // This will also redirect to /login
            this.logout();
          }
        });
      }
