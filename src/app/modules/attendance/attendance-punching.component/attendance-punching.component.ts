// attendance-punching.component.ts
...
export class AttendancePunchingComponent implements OnInit {
  ...
  // Grid Data
  punchHistoryData = signal<any[]>([]);
  historyColumns = signal<any[]>([]);
  ...
  setupGridColumns(): void {
    const columns = [
      {
        ...
        cellRenderer: (params: any) => {
          return `
            <div style="display:flex; align-items:center; height:100%; font-weight:600; color:var(--text-primary); font-size:12px;">
              ${this.commonService.formatDate(params.value, 'dd MMM yyyy')}
            </div>`;
        },
        sortable: true,
        filter: true
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          return `
            <div style="display:flex; align-items:center; height:100%; font-family:var(--font-mono, monospace); font-size:12px; color:var(--text-secondary);">
              ${checkIn}
            </div>`;
        }
      },
      ...
      },
      {
        ...
        width: 80,
        cellRenderer: (params: any) => {
          const breakCount = params.data.breakCount || 0;
          return `
      ...
      },
      {
        ...
        width: 100,
        cellRenderer: (params: any) => {
          const hours = params.data.totalHours || 0;
          return `
            <div style="display:flex; align-items:center; height:100%; font-weight:bold; color:var(--text-primary); font-size:12px;">
              ${hours} hrs
            </div>`;
        }
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          const severity = params.data.statusSeverity || 'secondary';
        ...
        cellRenderer: (params: any) => {
          return `
            <div style="display:flex; align-items:center; justify-content:center; height:100%;">
              ...
              " onclick="this.dispatchEvent(new CustomEvent('viewDetails', { bubbles: true, detail: '${params.data.id}' }))">
                <i class="pi pi-eye"></i>
              </button>
            </div>`;
        }
    ...
    this.historyColumns.set(columns);
  }

  // Grid Event Handler
  eventFromGrid(event: any) {
    if (event.type === 'viewDetails') {
      const record = this.punchHistoryData().find(r => r.id === event.detail);
      if (record) {
        this.viewPunchDetails(record);
      }
    }
    ...
    if (event.type === 'reachedBottom') {
      // Handle infinite scroll if implemented
      console.log('Grid reached bottom');
    }
  }
  ...
  async detectLocation() {
    try {
      ...
      this.appMessageService.showError('Location Error', 'Unable to detect location');
    } finally {
  ...
  loadPunchHistoryGrid() {
    ...
    this.commonService.apiCall(
      ...
      (response) => {
        if (response?.data) {
          const history: any[] = response.data.map((record: any) => {
          ...
          this.punchHistoryData.set(history);
