...
@Component({
  ...
  template: `
    <div class="app-layout">
      ...
      <aside class="sidebar">
        ...
        <nav class="nav-menu">
          <button class="nav-item active" title="Shipments"><i class="pi pi-th-large"></i></button>
          <button class="nav-item" title="Scan" (click)="openScanner()"><i class="pi pi-qrcode"></i></button>
          <button class="nav-item" title="Settings" (click)="showPasswordModal.set(true)"><i class="pi pi-cog"></i></button>
        </nav>
          ...
          <div class="drawer-scroll-area">
            ...
            <div class="split-panels">
              <div class="panel-card flex-1">
                ...
                <div class="card-value d-flex-align">
                  <i class="pi pi-money-bill text-primary"></i>
                  <div>
                    <span class="main-val">{{ order.paymentMethod }}</span>
                    <span class="sub-val">{{ order.totalAmount | currency:'INR' }}</span>
                  </div>
                </div>
              </div>
              <div class="panel-card flex-1">
                <span class="card-label">Customer Contact</span>
                <div class="card-value d-flex-align">
                  <i class="pi pi-phone text-primary"></i>
                  <div>
                    <span class="main-val">{{ order.shippingAddress?.fullName }}</span>
                    <span class="sub-val">{{ order.shippingAddress?.phone }}</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  styles: [`
    ...
    .text-sm { font-size: var(--font-size-sm, 0.75rem); }
    .text-truncate { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: inline-block; }
    .w-100 { width: 100%; }
    .w-50 { width: 50%; }
    .flex-1 { flex: 1; }
    .d-flex { display: flex; }
    .d-flex-align { display: flex; align-items: center; gap: var(--spacing-md, 0.5rem); }
    .mt-sm { margin-top: var(--spacing-sm, 0.375rem); }
    .mt-md { margin-top: var(--spacing-xl, 1rem); }
    .mt-lg { margin-top: var(--spacing-3xl, 2rem); }
    .mb-md { margin-bottom: var(--spacing-xl, 1rem); }
    .gap-md { gap: var(--spacing-md, 0.5rem); }
    ...
    .btn-secondary {
      ...
      color: var(--text-primary, #111827);
      border: var(--ui-border-width, 1px) solid var(--border-secondary, #d1d5db);
      padding: var(--spacing-md, 0.5rem) var(--spacing-xl, 1rem);
      border-radius: var(--ui-border-radius-lg, 12px);
      font-weight: var(--font-weight-medium, 500);
      font-size: var(--font-size-md, 0.875rem);
    ...
    .btn-secondary:hover { background-color: var(--bg-secondary, #f3f4f6); }
    
    .btn-sm { padding: var(--spacing-sm, 0.375rem) var(--spacing-md, 0.5rem); font-size: var(--font-size-sm, 0.75rem); }
    
    .icon-btn {
      background: transparent; border: 1px solid var(--border-secondary, #e5e7eb);
      color: var(--color-primary, #2563eb); width: 32px; height: 32px;
    ...
    .sidebar {
      ...
      border-right: var(--ui-border-width, 1px) solid var(--border-secondary, #e5e7eb);
      display: flex; flex-direction: column; align-items: center;
      padding: var(--spacing-2xl, 1.5rem) 0;
      z-index: 10;
    }
    .brand-logo { color: var(--color-primary, #2563eb); font-size: 1.5rem; margin-bottom: var(--spacing-4xl, 2.75rem); }
    .nav-menu { display: flex; flex-direction: column; gap: var(--spacing-lg, 0.75rem); width: 100%; align-items: center; flex: 1; }
    .nav-item {
      background: transparent; border: none; color: var(--text-secondary, #9ca3af);
      width: 40px; height: 40px; border-radius: var(--ui-border-radius, 10px);
      font-size: 1.2rem; display: flex; align-items: center; justify-content: center;
      transition: var(--transition-fast, all 0.15s);
    }
    .nav-item:hover { color: var(--text-primary, #111827); background-color: var(--bg-secondary, #f3f4f6); }
    .nav-item.active { background-color: var(--color-primary-bg, #eff6ff); color: var(--color-primary, #2563eb); }
    ...
    .page-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--spacing-3xl, 2rem); }
    .header-title { display: flex; align-items: center; gap: var(--spacing-3xl, 2rem); }
    .header-title h1 { margin: 0; font-family: var(--font-heading, sans-serif); font-size: var(--font-size-3xl, 1.375rem); font-weight: var(--font-weight-bold, 700); }
    
    .segment-control {
      display: flex; background: var(--bg-primary, #ffffff);
      border: var(--ui-border-width, 1px) solid var(--border-secondary, #e5e7eb);
      border-radius: var(--ui-border-radius-pill, 9999px); padding: 4px;
    }
    .segment-control button {
      background: transparent; border: none; padding: 6px 16px; border-radius: var(--ui-border-radius-pill, 9999px);
      font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-medium, 500); color: var(--text-secondary, #6b7280);
    }
    .segment-control button.active { background: var(--bg-secondary, #f3f4f6); color: var(--text-primary, #111827); font-weight: var(--font-weight-semibold, 600); }

    .state-container { text-align: center; padding: 100px 20px; }
    .empty-icon { width: 64px; height: 64px; background: var(--bg-primary, #fff); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; color: var(--border-secondary, #d1d5db); margin: 0 auto 16px; box-shadow: var(--shadow-sm); }
    ...
    .shipment-card {
      ...
      border-radius: var(--ui-border-radius-lg, 16px);
      padding: var(--spacing-2xl, 1.5rem);
      box-shadow: var(--shadow-sm, 0 1px 2px rgba(0,0,0,0.05));
      border: var(--ui-border-width, 1px) solid var(--border-secondary, #f3f4f6);
      cursor: pointer; transition: var(--transition-base, all 0.2s);
      display: flex; flex-direction: column; gap: var(--spacing-xl, 1rem);
    }
    .shipment-card:hover { box-shadow: var(--shadow-md, 0 4px 12px rgba(0,0,0,0.08)); border-color: var(--border-primary, #e5e7eb); }
    
    .card-head { display: flex; justify-content: space-between; align-items: center; }
    .cargo-id { font-size: var(--font-size-md, 0.875rem); color: var(--text-secondary, #6b7280); }
    .cargo-id strong { color: var(--text-primary, #111827); font-family: var(--font-mono, monospace); }
    
    .status-badge {
      font-size: var(--font-size-xs, 0.65rem); font-weight: var(--font-weight-semibold, 600);
      padding: 4px 10px; border-radius: var(--ui-border-radius-pill, 9999px);
      background: var(--bg-secondary, #f3f4f6); color: var(--text-secondary, #6b7280); text-transform: uppercase;
    }
    .status-badge[data-status="shipped"] { background: var(--color-primary-bg, #eff6ff); color: var(--color-primary, #2563eb); }
    .status-badge[data-status="delivered"] { background: var(--color-success-bg, #dcfce7); color: var(--color-success, #059669); }
    .status-badge[data-status="unfulfilled"] { background: var(--color-warning-bg, #fef3c7); color: var(--color-warning, #d97706); }

    /* Route Visualizer */
    .route-overview { display: flex; align-items: flex-start; position: relative; margin: var(--spacing-md, 0.5rem) 0; }
    .route-point { display: flex; flex-direction: column; gap: 4px; flex: 1; z-index: 2; }
    .point-marker { width: 14px; height: 14px; border-radius: 50%; background: var(--bg-primary, #fff); border: 4px solid var(--border-secondary, #d1d5db); }
    .point-marker.origin { border-color: var(--color-primary, #2563eb); }
    .point-details { display: flex; flex-direction: column; }
    .date { font-size: var(--font-size-xs, 0.65rem); color: var(--text-secondary, #9ca3af); margin-bottom: 2px; }
    .location { display: flex; flex-direction: column; }
    .city { font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-semibold, 600); color: var(--text-primary, #111827); }
    .address { font-size: var(--font-size-xs, 0.65rem); color: var(--text-secondary, #6b7280); }
    
    .route-connector { position: absolute; top: 7px; left: 14px; right: 50%; height: 2px; background: repeating-linear-gradient(90deg, var(--border-secondary, #d1d5db) 0, var(--border-secondary, #d1d5db) 4px, transparent 4px, transparent 8px); z-index: 1; }

    /* Card Footer */
    .card-foot { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--border-secondary, #f3f4f6); padding-top: var(--spacing-xl, 1rem); }
    .client-info { display: flex; align-items: center; gap: var(--spacing-md, 0.5rem); }
    .avatar { width: 32px; height: 32px; border-radius: 50%; background: var(--color-primary-bg, #eff6ff); color: var(--color-primary, #2563eb); display: flex; align-items: center; justify-content: center; font-size: 0.8rem; }
    .client-details { display: flex; flex-direction: column; }
    .client-details .name { font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-medium, 500); }
    .client-details .role { font-size: var(--font-size-xs, 0.65rem); color: var(--text-secondary, #6b7280); }

    /* Side Details Drawer */
    .drawer-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.2); z-index: var(--z-modal-backdrop, 400); }
    .details-drawer {
      position: fixed; top: 0; right: 0; bottom: 0; width: 100%; max-width: 500px;
      background: var(--bg-primary, #ffffff); box-shadow: var(--shadow-2xl, -10px 0 30px rgba(0,0,0,0.1));
      z-index: var(--z-modal, 500); transform: translateX(100%); transition: transform 0.3s cubic-bezier(0.16, 1, 0.3, 1);
      display: flex; flex-direction: column;
    }
    .details-drawer.open { transform: translateX(0); }
    
    .drawer-header {
      padding: var(--spacing-2xl, 1.5rem); border-bottom: 1px solid var(--border-secondary, #e5e7eb);
      display: flex; justify-content: space-between; align-items: center;
    }
    .back-btn { background: transparent; border: none; font-size: var(--font-size-lg, 0.9375rem); font-weight: var(--font-weight-semibold, 600); display: flex; align-items: center; gap: 8px; color: var(--text-primary, #111827); }
    
    .drawer-scroll-area { padding: var(--spacing-2xl, 1.5rem); overflow-y: auto; flex: 1; display: flex; flex-direction: column; gap: var(--spacing-3xl, 2rem); }
    
    .panel-section { display: flex; flex-direction: column; gap: var(--spacing-md, 0.5rem); }
    .panel-section.bg-muted { background: var(--bg-secondary, #f9fafb); padding: var(--spacing-lg, 1rem); border-radius: var(--ui-border-radius, 10px); }
    .panel-title { font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-bold, 700); text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-main, #111827); margin-bottom: var(--spacing-sm, 0.5rem); }
    
    /* Order Summary Split Cards */
    .split-panels { display: flex; gap: var(--spacing-md, 1rem); }
    .panel-card { background: var(--bg-primary, #ffffff); border: 1px solid var(--border-secondary, #e5e7eb); border-radius: var(--ui-border-radius-lg, 12px); padding: var(--spacing-lg, 1rem); display: flex; flex-direction: column; gap: 8px; }
    .card-label { font-size: var(--font-size-xs, 0.65rem); text-transform: uppercase; color: var(--text-secondary, #6b7280); font-weight: var(--font-weight-semibold, 600); }
    .main-val { display: block; font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-semibold, 600); color: var(--text-primary, #111827); }
    .sub-val { display: block; font-size: var(--font-size-xs, 0.65rem); color: var(--text-secondary, #6b7280); }

    /* Timeline Details */
    .timeline-list { border: 1px solid var(--border-secondary, #e5e7eb); border-radius: var(--ui-border-radius-lg, 12px); overflow: hidden; }
    .tl-row { display: flex; align-items: center; padding: var(--spacing-md, 0.75rem) var(--spacing-lg, 1rem); border-bottom: 1px solid var(--border-secondary, #f3f4f6); background: var(--bg-primary, #fff); }
    .tl-row:last-child { border-bottom: none; }
    .tl-number { width: 24px; height: 24px; border-radius: 50%; background: var(--color-primary, #2563eb); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 700; margin-right: 12px; }
    .tl-data { display: flex; flex-direction: column; }
    .tl-city { font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-medium, 500); color: var(--text-primary, #111827); }
    .tl-address { font-size: var(--font-size-xs, 0.65rem); color: var(--text-secondary, #6b7280); }

    /* Cart Items */
    .cart-items { display: flex; flex-direction: column; gap: var(--spacing-sm, 0.5rem); }
    .cart-item { display: flex; align-items: center; gap: var(--spacing-md, 1rem); padding: var(--spacing-md, 0.75rem); border: 1px solid var(--border-secondary, #e5e7eb); border-radius: var(--ui-border-radius-lg, 12px); }
    .item-img-box { width: 48px; height: 48px; border-radius: var(--ui-border-radius, 8px); background: var(--bg-secondary, #f3f4f6); display: flex; align-items: center; justify-content: center; overflow: hidden; }
    .item-img-box img { width: 100%; height: 100%; object-fit: cover; }
    .item-info { flex: 1; display: flex; flex-direction: column; }
    .item-name { font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-semibold, 600); color: var(--text-primary, #111827); }
    .item-sku { font-size: var(--font-size-xs, 0.65rem); color: var(--text-secondary, #6b7280); }
    .item-price { font-size: var(--font-size-sm, 0.75rem); font-weight: var(--font-weight-bold, 700); }
    
    .note-text { font-size: var(--font-size-xs, 0.7rem); color: var(--text-primary, #374151); line-height: 1.4; margin: 0; }

    /* Modals */
    .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.3); z-index: var(--z-modal-backdrop, 400); display: flex; align-items: center; justify-content: center; padding: 16px; backdrop-filter: blur(2px); }
    .ui-modal { background: var(--bg-primary, #ffffff); border-radius: var(--ui-border-radius-xl, 20px); width: 100%; max-width: 420px; padding: var(--spacing-3xl, 2rem); box-shadow: var(--shadow-xl, 0 8px 24px rgba(0,0,0,0.12)); }
    
    .modal-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--spacing-xl, 1rem); }
    .modal-head h3 { margin: 0; font-size: var(--font-size-lg, 1.1rem); font-weight: var(--font-weight-bold, 700); }
    
    .ui-input { width: 100%; padding: 12px 16px; border-radius: var(--ui-border-radius, 10px); border: 1px solid var(--border-secondary, #d1d5db); background: var(--bg-primary, #fff); font-size: var(--font-size-sm, 0.875rem); box-sizing: border-box; outline: none; transition: border 0.2s; }
    .ui-input:focus { border-color: var(--color-primary, #2563eb); }
    
    .scanner-frame { border-radius: var(--ui-border-radius-lg, 12px); overflow: hidden; border: 1px solid var(--border-secondary, #e5e7eb); }
    .error-msg { color: var(--color-error, #dc2626); font-size: var(--font-size-sm, 0.75rem); text-align: center; }
    
    .modal-icon { width: 64px; height: 64px; border-radius: 50%; background: var(--color-primary-bg, #eff6ff); color: var(--color-primary, #2563eb); display: flex; align-items: center; justify-content: center; font-size: 2rem; margin: 0 auto; }
    .amount-display { font-size: var(--font-size-3xl, 2rem); font-weight: var(--font-weight-bold, 700); color: var(--text-primary, #111827); }

    /* Responsive */
    @media (max-width: 768px) {
      .sidebar { display: none; }
      .main-content { padding: var(--spacing-xl, 1rem); }
      .header-title { flex-direction: column; align-items: flex-start; gap: var(--spacing-md, 0.5rem); }
...
export class PlatformDashboardComponent implements OnInit, OnDestroy {
  ...
  // Safely compute the timeline for the details drawer
  selectedOrderTimeline = computed(() => {
