...
// // // FIX: Generate clear labels for nested paths
...
// //   `]
// // })
// // export class ExcelExportDialogComponent implements OnInit, OnDestroy {
...
// //     // Auto-build the dialog state when data comes in
// //     if (this._data && this._data.length > 0) {
// //       this.buildDialogState();
// //     }
// //   }
...
// FIX: Split the full path by dots so nested labels are distinct (e.g. "Department Id - Name")
function toLabel(path: string): string {
  return path.split('.')
    .map(key => key.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/[_.\-]+/g, ' ').replace(/\b\w/g, c => c.toUpperCase()).trim())
    .join(' - ');
}
...
function inferType(key: string, samples: unknown[]): ColumnType {
  ...
  if (ne.every(s => typeof s === 'boolean')) return 'boolean';
  if (BOOL_HINTS.some(h => low.startsWith(h))) return 'boolean';
  if (ne.every(s => typeof s === 'number' || !isNaN(Number(s)))) { // FIX: Added check for numeric strings
    if (PERCENT_HINTS.some(h => low.endsWith(h) || low === h)) return 'percent';
    if (CURRENCY_HINTS.some(h => low.includes(h))) return 'currency';
    return 'number';
  }
...
function walkRow(
  ...
  if (!obj || typeof obj !== 'object') return; // Safety check

  for (const [key, val] of Object.entries(obj)) {
    if (DEFAULT_SKIP_KEYS.has(key)) continue;
    const path = prefix ? `${prefix}.${key}` : key;
    const label = toLabel(path); // FIX: Use the full path for labeling
    ...
    if (Array.isArray(val)) {
      ...
      if (!hasObject) {
        ...
        const childFields = Array.from(cf.values()).map(f => ({ ...f, type: inferType(f.path.split('.').pop() ?? f.path, f.sample) }));
        arrays.set(path, { key: path, label: toLabel(path), childFields }); // FIX: Use full path for array label
      }
      continue;
    }
...
export class ExcelExportDialogComponent implements OnChanges, OnInit, OnDestroy {
  ...
  // FIX: Added custom getter/setter to extract your nested { data: { data: [] } } JSON flawlessly
  private _data: Record<string, unknown>[] = [];
    ...
    if (cfg.showTotals && data.length) {
      const totValues = allCols.map((col, i) => {
        ...
        if (['number', 'currency', 'percent'].includes(type) && col.showTotal !== false) {
          ...
          // FIX: Pre-calculate the result of the sum so Excel doesn't show 0
          let sum = 0;
          data.forEach(item => {
            const flat = flattenObject(item);
            const val = Number(getByPath(flat, col.key));
