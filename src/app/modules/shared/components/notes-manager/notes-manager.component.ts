...
// Services/Models (Preserving your imports)
...
export class NotesManagerComponent  implements OnInit {
  ...
  onMouseMove(event: MouseEvent) {
    const widget = this.elementRef.nativeElement.querySelector('.erp-glass-widget');
    if (!widget) return;
    ...
    // Only rotate if mouse is near/over the widget
    ...
    // Subtle 2-degree tilt for professional feel
  ...
  saveNote() {
    ...
    upload$.pipe(
      ...
      next: () => {
        ...
        this.messageService.add({ severity: 'success', summary: 'Saved', detail: 'Entry recorded.' });
        this.refreshAll();
      },
      error: () => this.showError('Failed to save entry')
    });
  }

  // --- MEDIA ---
  triggerFileUpload() { if (!this.isPreviewMode) this.fileInput.nativeElement.click(); }
