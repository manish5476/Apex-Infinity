  ...
  template: `
    @if (loadingService.isLoading$ | async) {
      <div class="loader-overlay">
        <!-- Only text elements, no boxes/containers -->
        <div class="text-stack">
          <div class="dynamic-text">{{currentText}}</div>
        ...
        <!-- Super subtle background effect only -->
        <div class="bg-effect"></div>
      </div>
    }
  `,
  styles: [`
    /* === Ultra Minimalist Loading === */
    ...
    .loader-overlay {
      ...
      &:before {
        ...
        background: 
          ...
          radial-gradient(circle at 80% 50%, rgba(168, 85, 247, 0.02) 0%, transparent 50%);
      }
    }

    /* Text stack - just floating text */
    .text-stack {
      position: relative;
      z-index: 10;
      text-align: center;
    ...
    .loading-dots {
      ...
      gap: 0.5rem;
      opacity: 0;
      animation: dotsFade 0.8s ease-out 0.3s forwards;
    }
    ...
    /* Super subtle background effect */
    .bg-effect {
      position: absolute;
      inset: 0;
      opacity: 0;
      animation: bgAppear 1s ease-out 0.5s forwards;
...
export class LoadingComponent implements OnInit, OnDestroy {
  ...
  rotateText() {
    let newText;
    do {
      newText = this.loadingTexts[Math.floor(Math.random() * this.loadingTexts.length)];
    } while (newText === this.currentText);
