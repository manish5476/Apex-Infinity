    ...
    <p-toast position="top-right" key="announcement" [baseZIndex]="9999">
      <ng-template let-message pTemplate="headless" let-closeFn="closeFn">
        ...
        <section class="announcement-card" [ngClass]="message.data.type || 'info'">
          ...
          <button class="close-btn" (click)="closeFn($event)" aria-label="Close Announcement">
...
export class AnnouncementListenerComponent {
  ...
  constructor() {
    ...
    this.socketConnection.announcement$
      .pipe(takeUntilDestroyed())
      .subscribe((announcement: AnnouncementData) => {
  ...
  playNotificationSound() {
    // Safety check for SSR or environments where audio isn't available
    if (typeof window !== 'undefined' && typeof Audio !== 'undefined') {
      try {
        ...
        audio.play().catch(() => { 
          ...
          // Silently catching this is standard practice.
        });
      } catch (e) { }
    }
  }
