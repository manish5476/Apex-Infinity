...
@Component({
  ...
  template: `
    <div class="shared-grid-container">
          ...
          <div class="toolbar-right">
            ...
            @if (!isBulkEditing()) {
              <button pButton label="Edit" icon="pi pi-pencil"
                size="small"
                styleClass="apex-btn btn-outline"
              ...
              @if (selectedCount() > 0) {
                <button pButton label="Delete" icon="pi pi-trash"
                  size="small"
                  styleClass="apex-btn btn-danger-soft"
              ...
              <button pButton icon="pi pi-check"
                size="small"
                styleClass="apex-btn btn-success"
                (click)="saveBulkEdit()">
              </button>
            }
          </div>
      ...
      <div class="grid-body">
        <ag-grid-angular
          class="ag-theme-quartz custom-apex-grid"
  ...
  styles: [`
    /* ══════════════════════════════════════════════════════
       SHARED GRID — APEX CRM Master Theme Integration
    ══════════════════════════════════════════════════════ */
    ...
    .shared-grid-container {
      ...
      overflow: hidden;
      box-shadow: var(--shadow-sm); /* Structural shadow */
    }
    ...
    .grid-toolbar {
      ...
      background: var(--bg-primary); /* Matched to screenshot */
    ...
    .selection-chip i { font-size: 0.75rem; }
    ...
    /* ── TOOLBAR BUTTONS (APEX TOKENIZED) ──────────────── */
    :host ::ng-deep {
      .apex-btn.p-button {
        font-family: var(--font-heading);
        font-size: var(--font-size-sm);
      ...
      .apex-btn.p-button:focus-visible {
        outline: var(--focus-outline-width) solid var(--accent-focus);
        outline-offset: var(--focus-ring-offset);
      }

      /* Light Soft Button (Add Row) */
      .apex-btn.btn-soft.p-button {
        background: var(--bg-secondary);
        border: var(--ui-border-width) solid var(--border-secondary);
        color: var(--text-secondary);
      }
      .apex-btn.btn-soft.p-button:hover {
        background: var(--bg-hover);
        color: var(--text-primary);
      }

      /* Green Success Button (Export Excel / Save) */
      .apex-btn.btn-success.p-button {
        background: var(--color-success);
        border: var(--ui-border-width) solid var(--color-success);
        color: #fff;
        box-shadow: var(--elevation-1);
      }
      .apex-btn.btn-success.p-button:hover {
        background: var(--color-success-dark);
        box-shadow: var(--elevation-2);
      }

      /* Outline Button (Edit / Cancel) */
      .apex-btn.btn-outline.p-button {
        background: transparent;
        border: var(--ui-border-width) solid var(--border-secondary);
        color: var(--text-secondary);
      }
      .apex-btn.btn-outline.p-button:hover {
        background: var(--bg-secondary);
        color: var(--text-primary);
      ...
      /* Danger Soft Button */
      .apex-btn.btn-danger-soft.p-button {
        background: var(--color-error-bg);
        border: var(--ui-border-width) solid transparent;
        color: var(--color-error);
      }
      .apex-btn.btn-danger-soft.p-button:hover {
        background: var(--color-error-light);
      }

      /* ── AG GRID APEX OVERRIDES ──────────────────────── */
      .custom-apex-grid {
        /* Kill default vertical borders to match screenshot */
        --ag-header-column-separator-display: none;
        --ag-header-column-resize-handle-display: block;
        --ag-header-column-resize-handle-color: var(--border-tertiary);
        
        /* Typography */
        --ag-font-family: var(--font-body);
        --ag-font-size: var(--font-size-sm);
        
        /* Padding to make it airy */
        --ag-cell-horizontal-padding: var(--spacing-lg);
      }

      .custom-apex-grid .ag-header {
        border-bottom: var(--ui-border-width) solid var(--border-tertiary);
      }

      .custom-apex-grid .ag-header-cell-text {
        font-family: var(--font-heading);
        font-size: var(--font-size-xs);
        font-weight: var(--font-weight-semibold);
        color: var(--text-tertiary);
      ...
      .custom-apex-grid .ag-row {
        border-bottom: var(--ui-border-width) solid var(--border-tertiary) !important;
        transition: background-color var(--transition-fast);
      }

      .custom-apex-grid .ag-cell {
        display: flex;
      ...
      .ag-cell[col-id="__actions__"]:focus {
        ...
        outline-offset: calc(var(--focus-ring-offset) * -1) !important;
        border-radius: var(--ui-border-radius-sm);
      }
    }
...
export class AppSharedGrid<T extends { _id?: string; id?: string }> {
  ...
  /* ── THEME (STRICTLY CONSUMING CANONICAL TOKENS) ────── */
  readonly agTheme = computed<Theme>(() =>
    themeQuartz.withParams({
      ...
      selectedRowBackgroundColor: 'var(--color-primary-bg)',
      // Adjusted heights to match the airy feel of the screenshot
      rowHeight: 52, 
      headerHeight: 46,
      spacing: 6,
      wrapperBorderRadius: '0px', // Container handles the outer radius
      rowBorder: { color: 'var(--border-tertiary)' }
    })
  );
}
