...
@Component({
      ...
      @for (node of nodes; track node.id || $index) {
        <div class="tree-node">
          <div class="node-content">
            ...
            <span class="node-label">{{ node.name || 'Department Node' }}</span>
          </div>
        </div>
      }
    </div>
  `,
  styles: [`
    .dept-tree-container { display: flex; flex-direction: column; gap: 8px; }
    .tree-node { 
      padding: 12px 16px; 
    ...
    .text-accent { color: var(--accent-primary); font-size: 1.2rem; }
    .node-label { font-size: var(--font-size-sm); font-weight: var(--font-weight-medium); color: var(--text-primary); }
  `]
})
