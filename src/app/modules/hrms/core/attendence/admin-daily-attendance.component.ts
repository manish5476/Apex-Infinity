...
export class AdminDailyAttendanceComponent implements OnInit {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.hrmsService.recalculateDaily(this.selectedDate).subscribe({
          next: () => {
            this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Recalculation completed.' });
            this.loadDailyRegister();
          },
          error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Recalculation failed.' })
        });
      }
    });
  }
  ...
  onExport() {
    ...
    this.hrmsService.exportAttendance({ fromDate: start, toDate: end, format: 'csv' }).subscribe({
      next: () => this.messageService.add({ severity: 'success', summary: 'Export Started', detail: 'Your CSV download will begin shortly.' }),
      error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to generate export.' })
    });
  }
