      ...
      <app-page-content [padded]="true">
        <div class="flex flex-row flex-wrap items-end gap-3 p-3.5 bg-[var(--bg-secondary)] border border-[var(--border-secondary)] rounded-xl shadow-xs w-full mb-3">
          ...
          <div class="flex flex-col gap-1.5 flex-1 min-w-[160px]">
            <label class="text-[11px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider pl-0.5">Rotation</label>
            <p-select [options]="rotationOptions" [(ngModel)]="filter.rotationType"
              [showClear]="true" placeholder="All Rotations" appendTo="body" styleClass="w-full h-[38px] text-sm rounded-lg" (onChange)="applyFilters()">
            </p-select>
          </div>
          ...
          <div class="flex flex-col gap-1.5 flex-1 min-w-[140px]">
            ...
            <p-select [options]="statusOptions" [(ngModel)]="filter.isActive"
              [showClear]="true" placeholder="All Statuses" appendTo="body" styleClass="w-full h-[38px] text-sm rounded-lg" (onChange)="applyFilters()">
            </p-select>
          </div>
