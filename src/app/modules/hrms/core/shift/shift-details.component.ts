...
@Component({
    ...
    <div class="app-fullscreen-wrapper fade-in">
      ...
      <header class="dashboard-header glass-header">
          ...
          @if (shift(); as sh) {
            <div>
              <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 4px;">
                ...
                <p-tag [value]="sh.isActive ? 'Active' : 'Inactive'" [severity]="sh.isActive ? 'success' : 'secondary'" [rounded]="true"></p-tag>
                
                @if (sh.isNightShift) {
                  <p-tag value="Night Shift" icon="pi pi-moon" severity="info" [rounded]="true" styleClass="bg-indigo-50 text-indigo-500 border-indigo-200"></p-tag>
                }
              </div>
        ...
        <div class="header-right">
          <p-button label="Close" [outlined]="true" severity="secondary" (onClick)="goBack()"></p-button>
          @if (shift()) {
            <p-button label="Edit Shift" icon="pi pi-pencil" (onClick)="editShift()"></p-button>
            <p-button label="ShiftAssignment" icon="pi pi-note" (onClick)="shiftAssignment()"></p-button>
          }
        </div>
      </header>
      ...
      <main class="dashboard-content">
        ...
        @if (shift(); as sh) {
          <div class="bento-grid">
            ...
            <p-card styleClass="bento-p-card span-2 card-anim-1 border-primary-accent">
            ...
            <p-card styleClass="bento-p-card card-anim-2">
              <ng-template pTemplate="title">
                  ...
                  <div class="tags-container mt-1">
                    @if (sh.weeklyOffs && sh.weeklyOffs.length > 0) {
                      @for (day of getFormattedDays(sh.weeklyOffs); track day) {
                        <p-tag [value]="day" severity="secondary" [rounded]="true"></p-tag>
                      }
                    } @else {
                  ...
                  <div class="tags-container mt-1">
                    @if (sh.applicableDays && sh.applicableDays.length > 0) {
                      @for (day of getFormattedDays(sh.applicableDays); track day) {
                        <p-tag [value]="day" severity="info" [rounded]="true"></p-tag>
                      }
                    } @else {
            ...
            <p-card styleClass="bento-p-card card-anim-3">
              <ng-template pTemplate="title">
            ...
            <p-card styleClass="bento-p-card card-anim-4">
              <ng-template pTemplate="title">
              ...
              <div class="flex-col gap-md">
                @if (sh.overtimeRules?.enabled) {
                  <p-tag value="Overtime Enabled" severity="success" styleClass="mb-2 w-max-content"></p-tag>
                  ...
                  <div class="empty-state-inline" style="padding: 1rem;">
                    <i class="pi pi-ban text-xl mb-2"></i>
                    <p class="m-0">Overtime is disabled for this shift.</p>
                  </div>
                }
              </div>
            </p-card>
            ...
            @if (sh.shiftType === 'flexi' && sh.flexiConfig) {
              <p-card styleClass="bento-p-card card-anim-5 border-success-accent bg-success-light">
                <ng-template pTemplate="title">
                  ...
                  <span>Flexi-Time Boundaries</span>
                </ng-template>
  ...
  styles: [`
    ...
    /* ==========================================================================
       PRIME NG CARD OVERRIDES (The Magic)
       ========================================================================== */
    ...
    :host ::ng-deep .bento-p-card .p-card-content {
      padding: 0;
      flex: 1;
      display: flex;
      flex-direction: column;
    ...
    :host ::ng-deep .border-success-accent { border-color: var(--color-success); }
    :host ::ng-deep .bg-success-light { background: color-mix(in srgb, var(--color-success) 2%, var(--component-bg)); }
    ...
    /* Visual Timing Block */
    .timing-block { display: flex; align-items: center; justify-content: space-between; padding: var(--spacing-lg) var(--spacing-xl); background: var(--component-surface-raised, var(--bg-secondary)); border: 1px solid var(--border-secondary); border-radius: var(--ui-border-radius-lg); }
    ...
    .info-group label { font-size: 0.6875rem; font-weight: var(--font-weight-semibold); color: var(--text-label, var(--text-tertiary)); text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    .color-error { color: var(--color-error, #dc2626); }
    .color-success { color: var(--color-success, #16a34a); }
    ...
    /* States */
    .empty-state-inline { display: flex; flex-direction: column; align-items: center; justify-content: center; color: var(--text-tertiary); text-align: center; border: 1px dashed var(--border-secondary); border-radius: var(--ui-border-radius-lg); background: var(--bg-primary); }
...
export class ShiftDetailsComponent implements OnInit, OnDestroy {
 ...
 shiftAssignment(): void {
  if (!this.shiftId) {
    console.warn('Shift ID missing');
    return;
  }

  this.router.navigate(['/hrms/shifts', this.shiftId, 'assignments']);
}
