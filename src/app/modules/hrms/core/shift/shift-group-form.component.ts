...
@Component({
  ...
  template: `
    ...
    <div class="page-container fade-in">
      ...
      <header class="page-header flex-between flex-wrap gap-md mb-4xl slide-down">
        <div class="flex align-items-center gap-xl">
          ...
          <div class="header-titles flex-col gap-xs">
            <h1 class="title font-heading text-3xl font-bold text-primary m-0 line-height-tight">
              {{ isEditMode() ? 'Edit Shift Group' : 'Create Shift Group' }}
            </h1>
            <p class="subtitle text-secondary text-md m-0">
              {{ isEditMode() ? 'Modify existing rotation rules.' : 'Define a new rotating shift pattern.' }}
            </p>
          </div>
        </div>
        ...
        <div class="header-actions flex align-items-center gap-md">
          <div class="header-status flex align-items-center gap-sm px-md py-xs border-radius-full border-1 border-solid" 
            ...
            <span class="text-xs font-bold uppercase tracking-widest">{{ form.valid ? 'Ready' : 'Draft' }}</span>
          </div>
          
          <p-button label="Cancel" [outlined]="true" severity="secondary" (onClick)="onCancel()" [disabled]="isSaving() || isLoading()"></p-button>
          <p-button 
            [label]="isSaving() ? (isEditMode() ? 'Updating...' : 'Saving...') : (isEditMode() ? 'Update Group' : 'Save Group')" 
      ...
      <main class="dashboard-content" [class.opacity-50]="isLoading()">
        
        @if (isLoading()) {
          <div class="bento-grid">
            <div class="span-2"><p-skeleton height="25rem" borderRadius="16px"></p-skeleton></div>
            <div><p-skeleton height="25rem" borderRadius="16px"></p-skeleton></div>
          ...
          <form [formGroup]="form" class="bento-grid pb-6">
            ...
            <p-card styleClass="glass-panel border-radius-xl shadow-sm h-full span-2 card-anim-1">
              <ng-template pTemplate="title">
                <div class="flex align-items-center gap-sm mb-sm border-bottom-subtle pb-sm">
                  <i class="pi pi-info-circle text-primary text-xl"></i>
                  <span class="font-heading font-bold text-lg text-primary">General Information</span>
                </div>
              </ng-template>
              
              <div class="inner-grid-2">
                <div class="form-field">
                  <label for="name" class="info-label">Group Name <span class="text-error">*</span></label>
                  <p-iconField iconPosition="left">
                    <p-inputIcon styleClass="pi pi-id-card text-tertiary"></p-inputIcon>
                    <input pInputText id="name" type="text" formControlName="name" placeholder="e.g. Nursing Rotation A" class="w-full" />
                  </p-iconField>
                </div>

                <div class="form-field">
                  <label for="code" class="info-label">System Code <span class="text-error">*</span></label>
                  <p-iconField iconPosition="left">
                    <p-inputIcon styleClass="pi pi-tag text-tertiary"></p-inputIcon>
                    <input pInputText id="code" type="text" formControlName="code" placeholder="e.g. NURS-ROT-A" class="w-full uppercase-input font-mono" />
                  </p-iconField>
                </div>

                <div class="form-field">
                  <label for="organizationId" class="info-label">Organization <span class="text-error">*</span></label>
                  <p-select 
                    id="organizationId" 
                ...
                <div class="form-field">
                  <label for="branchId" class="info-label">Branch Assignment</label>
                  <p-select 
                    id="branchId" 
                ...
                <div class="form-field">
                  <label for="effectiveFrom" class="info-label">Effective From</label>
                  <p-datepicker 
                    id="effectiveFrom" 
                ...
                <div class="form-field">
                  <label for="effectiveTo" class="info-label">Effective To</label>
                  <p-datepicker 
                    id="effectiveTo" 
                ...
                <div class="form-field span-2-inner mt-sm">
                  ...
                  <textarea 
                    ...
                    rows="3" 
                    [autoResize]="true" 
                    class="w-full line-height-relaxed" 
            ...
            <p-card styleClass="glass-panel border-radius-xl shadow-sm h-full card-anim-2 flex-col">
              <ng-template pTemplate="title">
                <div class="flex align-items-center gap-sm mb-sm border-bottom-subtle pb-sm">
                  <i class="pi pi-users text-primary text-xl"></i>
                  <span class="font-heading font-bold text-lg text-primary">Applicability</span>
                </div>
              </ng-template>
              
              <div class="flex-col gap-xl h-full">
                <p class="text-sm text-secondary m-0 line-height-relaxed">Select which departments and roles this group applies to.</p>
                
                <div class="form-field flex-col gap-xs">
                  <label for="applicableDepartments" class="info-label">Applicable Departments</label>
                  <p-multiSelect 
                ...
                <div class="form-field flex-col gap-xs">
                  <label for="applicableDesignations" class="info-label">Applicable Designations</label>
                  <p-multiSelect 
                ...
                <div class="status-toggle-wrapper mt-auto border-1 border-solid border-primary">
                  <label class="toggle-container m-0">
                    ...
                    <div class="toggle-text"><span class="font-bold text-sm text-primary">Group is Active</span></div>
                  </label>
                </div>
              </div>
            </p-card>

            <p-card styleClass="glass-panel border-radius-xl shadow-sm span-3 card-anim-3">
              <ng-template pTemplate="title">
                <div class="flex-between flex-wrap gap-md mb-sm border-bottom-subtle pb-sm">
                  <div class="flex align-items-center gap-sm">
                    <i class="pi pi-clock text-primary text-xl"></i>
                    <span class="font-heading font-bold text-lg text-primary">Shifts in Rotation</span>
                  </div>
                  <p-button size="small" [outlined]="true" severity="secondary" icon="pi pi-plus" label="Add Shift" (onClick)="addShift()"></p-button>
                </div>
              </ng-template>
              
              <p class="text-sm text-secondary m-0 mb-xl line-height-relaxed">Add the individual shifts that make up this rotation group. Assign a sequence number to define their order.</p>
              ...
              <div formArrayName="shifts" class="flex-col gap-md">
                @for (shiftCtrl of shiftsArray.controls; track $index) {
                  <div [formGroupName]="$index" class="array-row flex align-items-center gap-md p-md glass-inset border-radius-lg border-1 border-solid border-secondary fade-in">
                    ...
                    <div class="form-field m-0" style="width: 100px;">
                      <p-inputNumber 
                    ...
                    <p-button icon="pi pi-trash" severity="danger" [text]="true" [rounded]="true" (onClick)="removeShift($index)" pTooltip="Remove Shift"></p-button>
                  </div>
                }
            ...
            <p-card styleClass="glass-panel border-radius-xl shadow-sm span-3 card-anim-4">
              <ng-template pTemplate="title">
                <div class="flex-between flex-wrap gap-md mb-sm border-bottom-subtle pb-sm">
                  <div class="flex align-items-center gap-sm">
                    <i class="pi pi-sync text-primary text-xl"></i>
                    <span class="font-heading font-bold text-lg text-primary">Rotation Pattern</span>
                  </div>
                  <p-button size="small" [outlined]="true" severity="secondary" icon="pi pi-plus" label="Add Pattern Rule" (onClick)="addRotationPattern()"></p-button>
                </div>
              </ng-template>

              <div class="inner-grid-3 mb-xl">
                <div class="form-field flex-col gap-xs">
                  <label for="rotationType" class="info-label">Rotation Frequency <span class="text-error">*</span></label>
                  <p-select 
                    id="rotationType" 
  ...
  styles: [`
    ...
    .bento-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--spacing-2xl); align-items: start; }
    ...
    .opacity-50 { opacity: 0.5; pointer-events: none; filter: grayscale(50%); }
    ...
    /* ==========================================================================
       COMPONENT SPECIFICS
       ========================================================================== */
    .info-label { font-size: var(--font-size-xs); font-weight: var(--font-weight-bold); color: var(--text-label, var(--text-tertiary)); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px; display: block; }
    
    /* Header Status Indicator */
    .valid-status { color: var(--color-success); border-color: color-mix(in srgb, var(--color-success) 30%, transparent); background: color-mix(in srgb, var(--color-success) 10%, transparent); }
    .valid-status .status-dot { background: var(--color-success); }
    
    .invalid-status { color: var(--text-tertiary); border-color: var(--border-secondary); background: var(--bg-secondary); }
    .invalid-status .status-dot { background: var(--text-tertiary); }
    
    .status-dot { width: 8px; height: 8px; border-radius: 50%; }

    /* Custom CSS Toggle Switch (No ng-deep needed) */
    .status-toggle-wrapper { padding: var(--spacing-md) var(--spacing-lg); background: color-mix(in srgb, var(--bg-secondary) 50%, transparent); border-radius: var(--ui-border-radius-md); }
    ...
    .color-picker-input::-webkit-color-swatch { border: none; border-radius: var(--ui-border-radius-sm); }

    /* Array Row Hover */
    .array-row:hover { border-color: var(--color-primary); box-shadow: var(--shadow-sm); }
...
export class ShiftGroupFormComponent implements OnInit {
  ...
  // Mock Lookups 
  organizations = [{ id: 'org_1', name: 'Main Organization' }];
  branches = [{ id: 'br_1', name: 'Head Office HQ' }];
  ...
  availableShifts = [
    { id: 'sh1', name: 'Morning Shift (08:00 - 16:00)' },
