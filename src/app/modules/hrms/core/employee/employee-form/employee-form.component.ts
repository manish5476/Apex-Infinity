...
export class EmployeeFormComponent implements OnInit, OnDestroy {
  ...
  private initForm() {
    ...
    const attConfig = this.userForm.get('employee.attendanceConfig') as FormGroup;
    ...
    this.userService.getUser(id)
      ...
      .subscribe({
        next: (res: any) => {
          ...
          if (user.employee) {
            user.employee.dateOfJoining = safeDate(user.employee.dateOfJoining);
            if (user.employee.personal) {
              user.employee.personal.dateOfBirth = safeDate(user.employee.personal.dateOfBirth);
            }
          }
          ...
          this.userForm.patchValue({
            ...
            employee: {
              ...user.employee,
              departmentId: extract(user.employee?.departmentId),
              designationId: extract(user.employee?.designationId),
              reportingManagerId: extract(user.employee?.reportingManagerId),
              personal: user.employee?.personal || {},
              bankDetails: user.employee?.bankDetails || {},
              guarantorDetails: user.employee?.guarantorDetails || {},
              attendanceConfig: {
                ...user.employee?.attendanceConfig,
                shiftId: extract(user.employee?.attendanceConfig?.shiftId),
                shiftGroupId: extract(user.employee?.attendanceConfig?.shiftGroupId),
                geoFenceId: extract(user.employee?.attendanceConfig?.geoFenceId)
              }
            }
          });
        },
