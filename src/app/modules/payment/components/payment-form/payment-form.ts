...
export class PaymentFormComponent implements OnInit, OnDestroy {
  ...
  invoiceOptions = signal<any[]>([]); 
  purchaseOptions = signal<any[]>([]); 
  ...
  private checkRouteForEditMode(): void {
    this.route.paramMap.pipe(
      switchMap(params => {
        ...
        if (this.paymentId) {
          // CRITICAL FIX: Uncommented this so onSubmit knows it's an update!
      ...
      next: (response) => {
        if (response && response.data && response.data.data) {
          ...
          this.messageService.showError('Failed to load payment data.');
        }
      },
      error: (err) => {
  ...
  onSubmit(): void {
    ...
    // Uncommented to prevent duplicate rapid submissions
    ...
    saveObservable.pipe(
      ...
      next: (res) => {
        ...
        this.messageService.showSuccess(`Payment ${this.editMode() ? 'updated' : 'recorded'} successfully.`);
