...
export class StorefrontLayoutComponent implements OnInit, OnDestroy {
  ...
  activeThemeStyle = computed<Record<string, string>>(() => {
    ...
    if (backendPreset) {
      ...
      primary = pageTheme?.customSettings?.primaryColor || globalSettings?.colors?.primary || '#2563eb';
      secondary = pageTheme?.customSettings?.secondaryColor || globalSettings?.colors?.secondary || '#475569';
    }
