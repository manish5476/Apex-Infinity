  ...
  template: `
    ...
    @if (searchOpen()) {
      <section class="overlay search-overlay" role="dialog" aria-modal="true" aria-label="Store search">
        ...
        <div class="search-box">
          ...
          <input autofocus type="search" placeholder="Search products, brands, categories…"
            [ngModel]="searchQuery()" (ngModelChange)="onSearch($event)" />
          <div class="search-results">
            @if (searchQuery().length < 2) {
              <p>Type at least two characters to search products.</p>
            } @else if (!searchResults().length) {
              <p>No instant matches yet — try a product name, SKU, or brand.</p>
            } @else {
              @for (product of searchResults(); track product._id || product.id || product.slug) {
                <a [routerLink]="['/store', slug(), 'products', product.slug || product._id || product.id]"
    ...
    @if (cartOpen()) {
      <aside class="cart-drawer" role="dialog" aria-modal="true" aria-label="Cart drawer">
        <div class="drawer-head">
          ...
          <button type="button" (click)="cartOpen.set(false)" aria-label="Close cart">
            <i class="pi pi-times"></i>
          </button>
        </div>
    ...
    @if (mobileOpen()) {
      <aside class="mobile-drawer" role="dialog" aria-modal="true" aria-label="Mobile menu">
        ...
        <nav>
          @for (item of menuItems(); track item.url || item.label) {
            <a [routerLink]="navLink(item.url)" (click)="mobileOpen.set(false)">
              {{ item.label }} <i class="pi pi-arrow-right"></i>
            </a>
          }
          <a [routerLink]="['/store', slug(), 'account']" (click)="mobileOpen.set(false)">
            Account <i class="pi pi-user"></i>
          </a>
          <a [routerLink]="['/store', slug(), 'cart']" (click)="mobileOpen.set(false)">
            Cart <i class="pi pi-shopping-bag"></i>
          </a>
        </nav>
      </aside>
