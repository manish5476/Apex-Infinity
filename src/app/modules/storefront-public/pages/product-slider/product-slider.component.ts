...
export class ProductSliderComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  private updateData() {
    ...
    this.products.set(sourceData.map(p => this.toPublicProduct(p)));
  }
  ...
  private toPublicProduct(p: SectionProduct): PublicProduct {
    return {
      ...
      image: p.image ?? null, images: p.images ?? [], tags: p.tags ?? [],
      price: p.price, stock: p.stock, category: p.category ?? null,
      categorySlug: p.categorySlug ?? null, brand: p.brand ?? null, brandSlug: p.brandSlug ?? null,
    };
  }
}
