...
export class ProductCardComponent {
  ...
  cardStyle() {
    const base: any = {};
    if (this.config()?.design?.borderRadius) {
      base['border-radius'] = `var(--ui-border-radius-${this.config().design.borderRadius})`;
    }
    if (this.config()?.design?.boxShadow && this.config().design.boxShadow !== 'none') {
      base['box-shadow'] = `var(--shadow-${this.config().design.boxShadow})`;
    }
    if (this.config()?.design?.customBackground) {
      base['background-color'] = this.config().design.customBackground;
    }
    return base;
  }

  imageShellStyle(direction: 'top' | 'left' = 'top') {
    if (!this.config()?.design?.borderRadius) return {};
    const radius = `var(--ui-border-radius-${this.config().design.borderRadius})`;
  ...
  headingStyle() {
    return {
      'font-family': this.config()?.typography?.headingFont || 'var(--font-heading)',
      'color': this.config()?.typography?.headingColor || 'var(--text-primary)'
    };
  }

  bodyStyle() {
    return {
      'font-family': this.config()?.typography?.bodyFont || 'var(--font-body)',
      'color': this.config()?.typography?.bodyColor || 'var(--text-secondary)'
    };
  }
  ...
  buttonStyle() {
    return {
      'font-family': this.config()?.typography?.headingFont || 'var(--font-heading)',
      'background-color': this.config()?.typography?.headingColor || 'var(--color-primary)',
  ...
  badgeStyle() {
    return {
      'font-family': this.config()?.typography?.bodyFont || 'var(--font-mono)',
      'color': this.config()?.typography?.headingColor || 'var(--color-primary)'
    };
  }
