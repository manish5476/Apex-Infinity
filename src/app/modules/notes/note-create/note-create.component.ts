...
@Component({
  ...
  styles: [`
    ...
    .page-header {
      ...
      .actions { display: flex; gap: var(--spacing-md); align-items: center; }
      .v-divider { width: 1px; height: 24px; background: var(--border-secondary); }
    }
    ...
    .btn-ghost {
      ...
      &.accent { color: var(--accent-primary); border: 1px solid var(--border-secondary); }
      &.accent:hover { background: var(--bg-ternary); border-color: var(--accent-primary); }
    }
...
export class NoteCreateComponent implements OnInit {
  ...
  applyTemplate(template: Note) {
    ...
    this.noteForm.patchValue({
      title: template.title, // Optional: might not want to overwrite title
    ...
    // Handle complex structures if needed (subtasks, etc)
    // For now, basic content patch is usually sufficient for templates
  }
  // ============================
