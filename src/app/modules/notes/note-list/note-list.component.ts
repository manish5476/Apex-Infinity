...
@Component({
      ...
      <main class="main-content">
        ...
        <header class="top-bar glass-panel">
          <div class="search-wrapper">
            ...
            <input type="text" [formControl]="searchControl" placeholder="Search notes, meetings, tags..." class="search-input">
          </div>
        ...
        <div class="content-area custom-scrollbar">
          ...
          <!-- Pagination -->
          @if (!isLoading() && totalPages() > 1 && activeFilter() !== 'trash' && activeFilter() !== 'shared') {
  ...
  styles: [`
    ...
    .filters-sidebar {
      ...
      .nav-menu {
        ...
        .nav-item {
          ...
          &.active { background: color-mix(in srgb, var(--primary) 10%, transparent); color: var(--primary); font-weight: 600; }
          ...
          .text { flex: 1; }
          .count { font-size: 10px; background: var(--bg-ternary); padding: 2px 6px; border-radius: 10px; }
        }
    ...
    .empty-state {
      ...
      .illustration { font-size: 3rem; margin-bottom: 16px; opacity: 0.5; }
...
export class NoteListComponent {
  ...
  // Users for sharing (Mock or fetched from MasterList)
  ...
  // Safely extracts numeric values from stats which might come as arrays from aggregation pipelines
  dashboardStats = computed(() => {
    ...
    // Extract Completed (Assuming 'completed' status exists in byStatus array)
    let completed = 0;
  ...
  constructor() {
    ...
    this.loadNotes();
    this.loadStats();
    // this.loadUsers(); // Implement if you have a user service
  }
  ...
  loadNotes() {
    ...
    // 1. Handle Trash Bin (Special Endpoint)
    if (filter === 'trash') {
      this.notesService.getTrashBin().subscribe({
        next: (res) => {
          this.notes.set(res.data.notes);
          this.isLoading.set(false);
          // Trash usually isn't paginated in simple implementations, or handled differently
    ...
    // 2. Handle Shared (Special Endpoint)
    if (filter === 'shared') {
    ...
    // 3. Handle Standard Filters (All, Favorites, Archived)
  ...
  setFilter(filter: 'all' | 'favorites' | 'shared' | 'archived' | 'trash') {
    ...
    this.currentPage.set(1);
    this.searchControl.setValue(''); // Reset search on filter change
    this.loadNotes();
  }
  ...
  onPinNote(id: string) {
    this.notesService.togglePinNote(id).subscribe(() => {
      ...
      // Reload if in favorites view to remove unpinned
  ...
  onShareNote(id: string) {
    // NoteCard handles the UI, this is just a hook if parent needs to know
    console.log('Shared note:', id);
  }

  onConvertToTask(id: string) {
    this.notesService.convertToTask(id).subscribe(() => {
       // Ideally refresh or update type locally
       this.loadNotes();
    });
  }

  // Linking Notes Dialog
...
// // Adjusted imports for standard Angular architecture
// // Ensure NoteService is at src/app/core/services/note.service.ts
// // Ensure Note Types are accessible
// import { Note, NoteStatistics, NoteFilterParams } from '../../../core/models/note.types';
// // Ensure NoteCardComponent is at src/app/components/note-card/note-card.component.ts
// import { NoteCardComponent, User } from '../note-card/note-card.component';
// import { NoteService } from '../../../core/services/notes.service';
...
//   // Linking Notes Dialog
//   onLinkNoteRequest(sourceId: string) {
//     alert('Feature available in full version: Open Link Note Dialog for ID: ' + sourceId);
//   }

//   exportNotes() {
//     this.notesService.exportNoteData('csv').subscribe(blob => {
//        // Handle blob download (simplified)
//        const url = window.URL.createObjectURL(blob);
//        const a = document.createElement('a');
//        a.href = url;
//        a.download = `notes_export_${new Date().toISOString()}.csv`;
//        a.click();
//     });
//   }
// }
...
// // Mock Dialog Service (Replace with your actual implementation)
...
//               <span class="icon"><i class="pi pi-file"></i></span> 
//               <span class="text">All Notes</span>
//               <span class="count" *ngIf="stats()">{{ stats()?.totalNotes || 0 }}</span>
//             </button>
//             <button class="nav-item" [class.active]="activeFilter() === 'favorites'" (click)="setFilter('favorites')">
//               <span class="icon"><i class="pi pi-star"></i></span> 
//               <span class="text">Favorites</span>
...
//           <div class="stat-row">
//             <span>Active</span>
//             <!-- <span class="val">{{ (stats()?.totalNotes || 0) - (stats()?.byStatus || 0) }}</span> -->
//           </div>
//            <div class="stat-row">
//             <span>Completed</span>
...
// export class NoteListComponent {
//   private notesService = inject(NoteService);
//   private dialogServices = inject(DynamicDialogServices); // Assuming this exists or is mocked
//   private router = inject(Router);
//   private fb = inject(FormBuilder);
...
//     if (this.dialogServices && this.dialogServices.openNoteLinkDialog) {
...
//                  this.notes.update(notes => notes.map(n => n._id === sourceId ? res.data.note : n));
//               }
//             });
//           }
//         });
//       }
//     } else {
//       console.warn('Dialog service for linking notes not implemented');
//       alert('Linking feature requires dialog service implementation');
//     }
//   }
...
// //       next: (res) => {        const updatedNote = res.data.note;
