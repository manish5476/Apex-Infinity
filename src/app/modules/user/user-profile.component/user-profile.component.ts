...
// Enhanced User Interface to match the template requirements
interface User {
  _id: string;
  name: string;
  email: string;
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DatePipe, SlicePipe, TitleCasePipe],
...
export class UserProfileComponent {
  ...
  onFileSelected(event: Event): void {
    ...
    if (!file || !file.type.startsWith('image/') || file.size > 5 * 1024 * 1024) {
      this.uploadError.set(file 
        ? (file.type.startsWith('image/') ? 'File size exceeds 5MB limit.' : 'Only image files are allowed.') 
        : null);
      return;
    }
    ...
    this.userService.uploadProfilePhoto(formData).pipe(
      ...
      next: (updatedUser) => {
        this.uploading.set(false);
        if (updatedUser) {
          // Merge generic user data with specific updates if needed, or just replace
