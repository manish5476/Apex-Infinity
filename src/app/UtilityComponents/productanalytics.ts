       ...
       <button class="close-btn" (click)="close()">
         <i class="pi pi-times"></i>
            ...
            <div class="product-identity">
              <span class="sku-badge">
                <i class="pi pi-barcode"></i> {{ d.product.sku || 'NO SKU' }}
              </span>
              <h2 class="product-title">{{ d.product.name }}</h2>
            </div>
            
            <div class="price-comparison">
              <div class="price-node cost">
                <span class="label">Cost</span>
                <span class="value">{{ d.product.purchasePrice | currency:'INR':'symbol':'1.0-0' }}</span>
              </div>
              <div class="connector"><i class="pi pi-arrow-right"></i></div>
              <div class="price-node sell">
        ...
        <div class="scrollable-content">
          ...
          <section class="kpi-section">
            ...
            <div class="kpi-card profit">
              ...
              <div class="kpi-content">
                ...
                <div class="kpi-badge">
                  <i class="pi pi-percentage"></i> {{ d.summary.profitMargin }}% Margin
                </div>
              </div>
            </div>
            ...
            <div class="kpi-card volume">
              ...
              <div class="kpi-content">
                ...
                <div class="kpi-value">{{ d.summary.totalQuantity }} <small>Units</small></div>
                <div class="kpi-sub">{{ d.summary.totalInvoices }} Orders</div>
              </div>
            </div>
            ...
            <div class="kpi-card avg">
              ...
              <div class="kpi-content">
                <span class="kpi-label">Avg. Profit / Unit</span>
                <div class="kpi-value neutral">{{ d.summary.profitPerUnit | currency:'INR':'symbol':'1.0-0' }}</div>
              </div>
            </div>
          </section>
          ...
          <section class="details-section">
            <p-tabs value="0">
              <p-tablist>
                <p-tab value="0"><i class="pi pi-history"></i> Recent Sales</p-tab>
                <p-tab value="1"><i class="pi pi-users"></i> Top Customers</p-tab>
                <p-tab value="2"><i class="pi pi-chart-bar"></i> Monthly Trend</p-tab>
              </p-tablist>
              
              <p-tabpanels>
                <p-tabpanel value="0">
                  <div class="table-wrapper">
                    <p-table [value]="d.recentSales" [scrollable]="true" scrollHeight="300px" styleClass="p-datatable-sm modern-table">
                      ...
                      <ng-template pTemplate="body" let-sale>
                        <tr>
                          <td><span class="mono-text">{{ sale.invoiceNumber }}</span></td>
                          <td class="text-secondary">{{ sale.invoiceDate | date:'dd MMM yyyy' }}</td>
                          <td class="text-right font-medium">{{ sale.quantity }}</td>
                          <td class="text-right text-success font-bold">{{ sale.profit | currency:'INR':'symbol':'1.0-0' }}</td>
                        </tr>
                      </ng-template>
                    </p-table>
                  </div>
                ...
                <p-tabpanel value="1">
                  <div class="table-wrapper">
                    <p-table [value]="d.customerAnalysis" [scrollable]="true" scrollHeight="300px" styleClass="p-datatable-sm modern-table">
                      <ng-template pTemplate="header">
                        <tr>
                          <th>Customer</th>
                          <th class="text-right">Qty</th>
                      ...
                      <ng-template pTemplate="body" let-cust>
                        <tr>
                          <td class="font-medium text-primary">{{ cust.customerName }}</td>
                          <td class="text-right">{{ cust.quantity }}</td>
                          <td class="text-right text-secondary">{{ cust.revenue | currency:'INR':'symbol':'1.0-0' }}</td>
                          <td class="text-right text-success font-bold">{{ cust.profit | currency:'INR':'symbol':'1.0-0' }}</td>
                        </tr>
                      </ng-template>
                    </p-table>
                  </div>
                ...
                <p-tabpanel value="2">
                  <div class="chart-wrapper">
                    @for(month of d.timeAnalysis; track month.month) {
                      <div class="chart-row">
                        <div class="row-label">{{ month.month }}</div>
                        <div class="row-bars">
                          <div class="bar-revenue" style="width: 100%">
                            <span class="bar-value">{{ month.revenue | currency:'INR':'symbol':'1.0-0' }}</span>
                          </div>
                          <div class="bar-profit" [style.width.%]="(month.profit / month.revenue) * 100">
                             @if((month.profit / month.revenue) > 0.2) {
                               <span class="bar-value-inner">{{ month.profit | currency:'INR':'symbol':'1.0-0' }}</span>
                             }
                          </div>
                        </div>
                      </div>
