  ...
  template: `
    <div class="apex-dg flex flex-col w-full" [class]="densityCssClass()">
      ...
      <div class="relative flex flex-col flex-1 min-h-0
        ...
        <!-- Scroll Container -->
        <div class="flex-1 overflow-auto apex-dg-scroll">

          <table class="w-full border-collapse table-auto apex-dg-table">
...
export class DataGridComponent implements OnDestroy {
  ...
  visibleColumnList = computed<GridColumn[]>(() => {
    ...
    return all.filter(c => vis.includes(c.field));
  });
  ...
  filteredData = computed<any[]>(() => {
    ...
    if (q) {
      const searchCols = this.columns().filter(c => c.searchable !== false && c.type !== 'action');
