...
@Component({
  ...
  template: `
    <header
      class="flex items-center h-[72px] px-6 flex-shrink-0"
      [class.justify-center]="isMini()"
      [class.justify-between]="!isMini()"
    >
      <!-- ── Brand ─────────────────────────────────────────── -->
      <div class="flex items-center gap-3 min-w-0 flex-1 cursor-default" [class.justify-center]="isMini()">
        
        <!-- Premium Star Logo Mark -->
        <div class="flex-shrink-0 flex items-center justify-center text-[var(--text-primary)]">
        ...
        <!-- Menu Text -->
        @if (!isMini()) {
          <div class="flex flex-col min-w-0 select-none pt-1">
            <span class="text-[17px] font-[var(--font-weight-bold)] text-[var(--text-primary)] tracking-tight leading-none truncate">
              Menu
            </span>
          </div>
        }
      ...
      @if (!isMini()) {
        <div class="flex items-center flex-shrink-0">
          @if (!isMobile()) {
            <app-icon-btn icon="pi pi-angle-double-left" tooltip="Collapse menu" (clicked)="toggleMini.emit()" />
          }
          @if (isMobile()) {
            <app-icon-btn icon="pi pi-times" tooltip="Close menu" (clicked)="closeMobile.emit()" />
          }
        </div>
      } @else {
        <div class="mt-4">
          <app-icon-btn icon="pi pi-angle-double-right" tooltip="Expand menu" tooltipPosition="right" (clicked)="toggleMini.emit()" />
        </div>
      }
    </header>
  `,
...
//   template: `
//     <header
//       class="flex items-center h-[48px] px-3 flex-shrink-0 border-b border-[var(--border-primary)]"
//       [class.justify-center]="isMini()"
//       [class.justify-between]="!isMini()"
//     >
//       <!-- ── Brand ─────────────────────────────────────────── -->
...
//       >
//         <!-- Logo mark -->
//         <div class="w-6 h-6 flex-shrink-0 rounded-[6px] flex items-center justify-center
//                     bg-[var(--accent-primary)] text-white shadow-sm font-semibold text-[13px] tracking-tight">
//           <svg viewBox="0 0 32 32" fill="none" class="w-[12px] h-[12px]" aria-hidden="true">
...
//                          text-[var(--text-primary)] tracking-tight leading-none truncate">
//               Apex
//               <span class="text-[var(--text-tertiary)] font-normal">Infinity</span>
//             </span>
//             @if (workspaceName()) {
//               <span class="text-[11px] leading-none text-[var(--text-muted)] truncate mt-1">
//                 {{ workspaceName() }}
//               </span>
//             }
//           </div>
