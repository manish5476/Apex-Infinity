...
@Component({
  ...
  template: `
    <div class="mb-4 px-3">
      <!-- Muted, minimal section spacer/label -->
      @if (!isMini() && group().groupLabel) {
        <div class="text-[12px] font-[var(--font-weight-semibold)] text-[var(--text-muted)] opacity-60 px-4 mb-2 mt-4 select-none" aria-hidden="true">
          {{ group().groupLabel }}
        </div>
      } @else {
...
//       @if (!isMini()) {
//         <div class="text-[11px] font-[var(--font-weight-semibold)] tracking-[0.08em] text-[var(--text-muted)] uppercase px-2 mb-1 mt-2 select-none" aria-hidden="true">
//           {{ group().groupLabel }}
//         </div>
//       } @else {
