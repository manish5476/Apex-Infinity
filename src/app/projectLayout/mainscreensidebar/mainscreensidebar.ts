...
import { COMPACT_MENU, MenuItem } from './menu-items.constants';
...
export class MainscreenSidebar implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Using compact menu version
  menuItems: MenuItem[] = COMPACT_MENU;
  
  expandedItems: Record<string, boolean> = {};
  focusedIndex = -1;
  private routerSubscription!: Subscription;
  
  // Track recent items for quick access
  ...
  ngAfterViewInit() {
    // Scroll to active item after view initialization
  ...
  toggleMenuItem(label: string) {
    if (!this.layout.isExpanded() && !this.layout.isMobile()) {
      this.layout.isHovered.set(true);
      setTimeout(() => {
  ...
  getSubmenuHeight(item: MenuItem): string {
    ...
    // 40px per submenu item
    return `${item.items.length * 40}px`;
  }
  ...
  isParentActive(item: MenuItem): boolean {
    ...
    return item.items.some(subItem => 
      subItem.routerLink && this.router.url.startsWith(subItem.routerLink[0])
    );
  }

  private autoExpandActiveRoute() {
    // Close all first if in single expand mode
    if (this.layout.config().sidebarSingleExpand) {
      Object.keys(this.expandedItems).forEach(key => {
        this.expandedItems[key] = false;
      });
  ...
  private loadRecentItems() {
    try {
      ...
      if (saved) {
        this.recentItems = JSON.parse(saved).slice(0, 5);
      }
    } catch (e) {
  ...
  private updateRecentItems(url: string) {
    // Find the current route in menu items
    const findRoute = (items: MenuItem[]): { label: string, routerLink: string[] } | null => {
      for (const item of items) {
        if (item.routerLink && url.startsWith(item.routerLink[0])) {
          return { label: item.label, routerLink: item.routerLink };
        }
        if (item.items) {
          const found = findRoute(item.items);
    ...
    const route = findRoute(this.menuItems);
    if (route) {
      // Remove if already exists
      this.recentItems = this.recentItems.filter(item => 
        item.routerLink[0] !== route.routerLink[0]
      );
      
      // Add to beginning
      this.recentItems.unshift(route);
  ...
  onKeyNavigate(event: KeyboardEvent) {
    if (!this.layout.isExpanded() && !this.layout.isMobile()) return;
    ...
    switch (event.key) {
      case 'ArrowDown':
        ...
        this.focusedIndex = (this.focusedIndex + 1) % focusableElements.length;
        focusableElements[this.focusedIndex]?.focus();
        break;
        
      case 'ArrowUp':
        event.preventDefault();
        this.focusedIndex = (this.focusedIndex - 1 + focusableElements.length) % focusableElements.length;
        focusableElements[this.focusedIndex]?.focus();
        break;
      ...
      case ' ':
        ...
        if (this.focusedIndex !== -1) {
          focusableElements[this.focusedIndex]?.click();
        }
        break;
      ...
      case 'End':
        event.preventDefault();
        this.focusedIndex = focusableElements.length - 1;
        focusableElements[this.focusedIndex]?.focus();
        break;
    }
  }
  ...
  private getFocusableElements(): HTMLElement[] {
    return Array.from(
      this.menuList?.nativeElement?.querySelectorAll(
        'a[tabindex="0"], .menu-title[tabindex="0"]'
      ) || []
    ) as HTMLElement[];
  }
  ...
  logout() {
    ...
    this.recentItems = [];
    localStorage.removeItem('sidebar_recent_items');
  ...
  isActiveRoute(routerLink: string[] | undefined): boolean {
    ...
    return this.router.url.startsWith(routerLink[0]);
  }

  shouldShowItem(item: MenuItem): boolean {
    // Add permission-based filtering here if needed
    // Example: return this.userService.hasPermission(item.permission);
    return true;
  }
}
// import { Component, ElementRef, inject, OnInit, ViewChild, AfterViewInit, OnDestroy } from '@angular/core';
