...
import { NotificationService } from '../../core/services/notification.service'; // Import this back
...
export class MainscreenHeader implements OnInit, OnDestroy {
  ...
  showNotificationdialog: boolean = false; // Restore this for the modal
  recentNotifications: any[] = [];         // Restore this for the red dot badge
  ...
  ngOnInit() {
    ...
    // 2. Subscribe to Notifications (For the Red Dot Badge)
    this.notificationService.notifications$
      .pipe(takeUntil(this.destroy$))
      .subscribe(notifications => {
        // Filter for unread notifications to determine if red dot shows
        this.recentNotifications = notifications.filter(n => !n.isRead);
      });

    // 3. Subscribe to Theme Settings
